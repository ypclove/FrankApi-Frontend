import {
  ...
  getInterfaceListBySearchTextPageUsingGet
...
const InterfaceSquare: React.FC = () => {
  ...
  const loadData = async (current = 1) => {
    setLoading(true);
    const res = await getInterfaceListByPageUsingGet({
      current: current,
      name: searchText,
      pageSize: pageSize,
  ...
  const onSearch = async () => {
    const res = await getInterfaceListBySearchTextPageUsingGet({
      current: 1,
