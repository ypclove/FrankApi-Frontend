...
import {
  closeProductOrderUsingPost,
  deleteProductOrderUsingDelete,
...
export default () => {
  ...
  const loadData = async (orderId: string | undefined) => {
    if (!params.id) {
      message.error('参数不存在');
      return;
    }
   ...
   * @param orderId
   */
  const handleClosed = async (orderId: string | undefined) => {
    const hide = message.loading('正在取消订单');
    // if (!record) return true;
    try {
      const res = await closeProductOrderUsingPost(orderId);
      hide();
      if (res.data) {
  ...
  const handleDelete = async (orderId: string | undefined) => {
    ...
    // if (!record) return true;
    try {
      const res = await deleteProductOrderUsingDelete(orderId);
      hide();
      if (res.data) {
      ...
      >
        <Descriptions>
          ...
          <Descriptions.Item key={'productType'} label="商品类别">
            {data && data.productType === 3 && '充值活动'}
            {data && data.productType === 2 && '积分充值'}
            {data && data.productType === 1 && 'VIP会员'}
          </Descriptions.Item>
